...
class Model():
    ...
    def build(self):
        ...
        x = self._pool_layer(name='pool1', inp=x, ksize=2, stride=2, mode='LP',p=8) # Nx16x16x16
        ...
        x = self._conv_layer(name='conv22', inp=x, kernel_shape=[3, 3, 32, 32],
                             stride=1, is_training=is_training)
        x = self._pool_layer(name='pool2', inp=x, ksize=2, stride=2, mode='LP',p=8) # Nx8x8x32
        ...
        x = self._conv_layer(name='conv32', inp=x, kernel_shape=[3, 3, 64, 64],
                             stride=1, is_training=is_training)
        x = self._pool_layer(name='pool3', inp=x, ksize=2, stride=2, mode='LP',p=8) # Nx4x4x64
